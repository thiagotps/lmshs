...
collectFromExpr :: KernelConfig -> Expr -> [(STVar, Expr)]
collectFromExpr config (Expr m) = A.toList . getSTSum $ s
  where
    s =  mconcat [mulSTSum (reduce t) n | (t, n) <- A.toList m]
    reduce = reduceTerm config
    mulSTSum (STSum m) n = STSum . A.map (* fromIntegral n) $ m

expand :: KernelConfig -> STVar -> [(STVar, Expr)]
...
kernel config rootList =
  let (stateVars, levelSize) = runRS go (S.fromList . map normalize $ rootList, S.empty) []
  ...
  where
    go :: RS (Set STVar, Set STVar) SparseSymbolic [Int]
    go = do
      (visit, seen) <- ask
...
buildEvaluatorInKernel config stateVarList NumericalConfig{numericExpandF} expr = eval
  where
        (iniSTVlist, vec) = let l = second (expr2Double numericExpandF) <$> collectFromExpr config expr

...
import Data.Either
import Data.Maybe
import LinearAlgebra.Sparse (CompInfo (..), EigenConfig (..), SparseMatrix, eigenDefaultConfig, maxEigenValue, powerMaxEigenValue, (!*.))
import LinearAlgebra.Vector (VectorDouble, (.+.))
import Model.Classic (ModelConfig (..), NumericalModelConfig (..), buildExpr, buildKernelConfig, buildNumericalConfig, runModel, simpleRunModel)
import Symbolic.Kernel (KernelOutput (..), NumericalConfig (..), NumericalMatrices (NumericalMatrices, matrixA, vectorB, vectorY0), SparseSymbolic, buildEvaluator, buildNumMatrices, kernelExpr, simpleKernelExpr)
...
isMatrixGood BinarySearchConfig {useSpectra} m =
  (\b -> abs b < 1.0)
    <$> ( case powerMaxEigenValue m of
...
binarySearch bsc@BinarySearchConfig {precision} = bs
  where
    check mid =
      let m = buildCustomMatrix bsc mid
       in case isMatrixGood bsc m of
            Right r -> Right r
            Left info -> Left (m, info)

    bs :: Double -> Double -> Either (SparseMatrix, CompInfo) Double
    bs low high
      | abs (low - high) < precision = Right mid
      | otherwise = do
        ck <- check mid
        if ck
          then bs mid high
...
runProgram Complete {..} ncpu = do
  if
      | filterLenght <= 0 -> putStrLn "filter lenght must be greater than 0"
